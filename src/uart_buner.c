...
{
	platform_printf("event Burn start\r\n"); 
...
const unsigned char auchCRCHi918[] =
{
	0x00, 0xC1, 0x81, 0x40, 0x01, 0xC0, 0x80, 0x41, 0x01, 0xC0, 0x80, 0x41, 0x00, 0xC1, 0x81, 0x40,
	0x01, 0xC0, 0x80, 0x41, 0x00, 0xC1, 0x81, 0x40, 0x00, 0xC1, 0x81, 0x40, 0x01, 0xC0, 0x80, 0x41,
	0x01, 0xC0, 0x80, 0x41, 0x00, 0xC1, 0x81, 0x40, 0x00, 0xC1, 0x81, 0x40, 0x01, 0xC0, 0x80, 0x41, 
...
const unsigned char auchCRCLo918[] = 
{
...
unsigned short check_crc16_918(uint8_t  *puchMsg , uint16_t usDataLen)
{
    ...
    {
        uIndex = uchCRCHi ^ *puchMsg++ ; /* calculate the CRC */
        uchCRCHi = uchCRCLo ^ auchCRCHi918[uIndex];
        uchCRCLo = auchCRCLo918[uIndex] ;
    }
...
//uint8_t *Buf = (uint8_t*)0x2000E000;
        ...
        switch(*(uint16_t*)cmd_event)
        {
            ...
            case EVENT_ID_UART_BURN_START:
            {
				...
				if(NULL == fp || NULL == Buf ) {
					...
					fp = NULL;
					Buf = NULL;
					UserQue_SendMsg(USER_MSG_BURN_STATE,NULL,BURN_STATE_MEM_OVER);
					break;
				}
				...
				if (res != FR_OK) {
					...
					fp = NULL;
					Buf = NULL;
					UserQue_SendMsg(USER_MSG_BURN_STATE,NULL,BURN_STATE_FILE_ERR);
					break;
				}
...
//							}
                    ...
                    {          
						...
						{
							ing_buner.total_burn_data_size += bytesRead;
							crc = check_crc16_918(Buf,bytesRead);
							driver_append_tx_data(Buf, bytesRead/2);
							vTaskDelay(pdMS_TO_TICKS(20));
							driver_append_tx_data(Buf+bytesRead/2, bytesRead/2);
							driver_append_tx_data((uint8_t *)&crc, 2);
							ing_buner.uart_burn_step = 2;
							 ing_buner.pag_index += 1;  
							//printf("crc:%x\r\n",crc);
            ...
            break;    
             ...
             {              
                ...
                UserQue_SendMsg(USER_MSG_BURN_STATE,NULL,BURN_STATE_NAK);
				f_close(fp);
				free(fp);	
				free(Buf);
				fp = NULL;
              ...
              case EVENT_ID_UART_BURN_LOCK:
             {
                ...
                {  
                    ing_buner.uart_burn_step = 0;
					UserQue_SendMsg(USER_MSG_BURN_STATE,NULL,BURN_STATE_FLASH_LOCK);
                }
             ...
             case EVENT_ID_UART_UNKNOW_CMD:
             {
                 if(ing_buner.uart_burn_step)
                 {
					...
					fp = NULL;
					Buf = NULL;
                    UserQue_SendMsg(USER_MSG_BURN_STATE,NULL,BURN_STATE_UNKNOW_CMD);
                 }
             }
                break;                 
            default:break;            
